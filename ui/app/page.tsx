...
function ReceiptScanFigure() {
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_10rem]">
      <div className="rounded-[0.9rem] border border-[var(--line)] bg-white p-5 shadow-[0_16px_36px_rgba(14,18,24,0.04)]">
        <div className="flex items-center justify-between">
          <h3 className="text-3xl leading-none text-[var(--foreground)]">
            Receipt scan
          </h3>
          <div className="flex h-11 w-11 items-center justify-center rounded-full border border-[var(--line)]">
            <Camera className="h-5 w-5" />
          </div>
        </div>

        <div className="relative mt-6 h-[18rem] overflow-hidden rounded-[0.85rem] border border-[var(--line)] bg-[#fbfbfd] px-5 py-5">
          <div className="space-y-3">
            {Array.from({ length: 9 }).map((_, index) => (
              <div
                key={index}
                className="flex items-center justify-between border-b border-[#eef0f4] pb-3 last:border-b-0"
              >
                <span
                  className="h-3 rounded-full bg-[#e6e8ef]"
                  style={{ width: `${4.4 + (index % 4) * 1.35}rem` }}
                />
                <span
                  className="h-3 rounded-full bg-[#d8dbe4]"
          ...
          <div className="pointer-events-none absolute inset-x-11 top-[5.2rem] h-9 rounded-full bg-[#efedff]/80" />
          <div className="pointer-events-none absolute inset-x-12 top-[9.25rem] h-10 rounded-full bg-[#dff4e8]/90" />
          <motion.div
            animate={{ y: [0, 198, 0] }}
      ...
      <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
        <div className="rounded-[0.8rem] bg-[#cfccff] p-5 text-[#050506]">
          <Eyebrow>Items found</Eyebrow>
          <p className="mt-3 text-3xl font-semibold leading-none">8</p>
        </div>

        <div className="rounded-[0.8rem] border border-[var(--line)] bg-white p-4">
          <Eyebrow>Extracted</Eyebrow>
          <div className="mt-4 space-y-2 text-sm">
            {["Tax $3.10", "Tip $4.00", "Total $104.50"].map((item) => (
              <p
                key={item}
                className={`rounded-full border border-[var(--line)] px-3 py-2 ${item.startsWith("Total") ? "bg-[#9bdab5]" : "bg-white"}`}
              >
                {item}
              </p>
            ))}
        ...
        <div className="rounded-[0.8rem] border border-[var(--line)] bg-white p-5">
          <Eyebrow>Detected line item</Eyebrow>
          <p className="mt-4 text-2xl leading-tight">House noodles</p>
...
function GroupIconGrid() {
  ...
  return (
    <div className="mt-8 grid grid-cols-2 gap-x-8 gap-y-6 text-sm">
      {items.map((item) => {
        const Icon = item.icon;
...
function AssignPeopleFigure() {
  ...
  return (
    <div className="rounded-[0.9rem] border border-[var(--line)] bg-white p-6 shadow-[0_16px_36px_rgba(14,18,24,0.04)] lg:min-h-[34rem]">
      <h3 className="text-3xl leading-none text-[var(--foreground)]">
        Assign people
      </h3>

      <div className="mt-5 grid gap-5 lg:min-h-[27rem] lg:grid-cols-[1fr_0.72fr_8.5rem] lg:items-stretch">
        <div className="space-y-4 lg:grid lg:grid-rows-2 lg:gap-4 lg:space-y-0">
          {groups.map((group) => (
            <div
              key={group.label}
              className="relative overflow-hidden rounded-[0.8rem] border border-[var(--line)] px-5 py-4 lg:flex lg:flex-col lg:justify-between"
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2 lg:mt-8">
                {group.people.map((person) => (
                  <span
                    key={person.name}
                    className={`rounded-full border border-[var(--line)] px-4 py-2 text-sm font-medium ${person.active ? "bg-[#050506] text-white" : "bg-[#f7f8fb] text-[var(--foreground)]"}`}
                  >
                    {person.name}
                  </span>
                ))}
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-1 lg:grid-rows-2">
          {[
            ["Groups", "2"],
            ["People", "5"],
          ].map(([label, value]) => (
            <div
              key={label}
              className="rounded-[0.8rem] bg-[#cfccff] p-5 text-[#050506]"
            >
...
function SavedSplitFigure() {
  return (
    <div className="grid gap-5 md:grid-cols-[0.45fr_1fr]">
      <div className="rounded-[0.9rem] border border-[var(--line)] bg-white p-7 shadow-[0_16px_36px_rgba(14,18,24,0.04)]">
        <Eyebrow>Settled total</Eyebrow>
        <p className="mt-7 text-5xl font-semibold leading-none">$189</p>
        <Eyebrow>
          <span className="mt-12 block">People paid</span>
        </Eyebrow>
        <p className="mt-7 text-5xl font-semibold leading-none">9/9</p>
      </div>

      <div className="rounded-[0.9rem] border border-[var(--line)] bg-white p-6 shadow-[0_16px_36px_rgba(14,18,24,0.04)]">
        <div className="flex flex-col gap-4 border-b border-[var(--line)] pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h3 className="mt-4 text-3xl leading-none">Bar Clara</h3>
            <p className="mt-3 text-sm text-[var(--muted-foreground)]">
              Friday dinner settled and saved.
            </p>
          </div>

          <div className="rounded-[0.8rem] bg-[#e2f5e8] px-5 py-4 text-[#116149]">
            <p className="text-[0.68rem] font-semibold uppercase tracking-[0.2em]">
              Payment status
            </p>
            <p className="mt-3 font-semibold">Everyone paid</p>
            <p className="mt-2 text-xs font-medium">3 of 3 settled</p>
          </div>
        </div>

        <div className="mt-5 rounded-[0.8rem] border border-[var(--line)]">
          <div className="flex items-center justify-between gap-4 border-b border-[var(--line)] px-5 py-4">
            <div>
              <Eyebrow>Split total</Eyebrow>
              <p className="mt-2 text-xl font-semibold">$84.20</p>
            </div>
            <span className="rounded-full bg-[#e2f5e8] px-3 py-1.5 text-xs font-semibold text-[#116149]">
              All settled
            </span>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-[#17a765]">
                      Paid
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-semibold leading-none">{person.amount}</p>
                  <p className="mt-2 text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-[var(--muted-foreground)]">
                    Received
                  </p>
                </div>
              </div>
